use crate::llm::{AnthropicConfig, OpenAIConfig};
...
impl LLMConfig {
    ...
    pub fn get_current_provider(&self) -> Result<&ProviderConfig> {
        self.providers.get(&self.provider).ok_or_else(|| {
            DoumError::Config(format!("프로바이더 '{}'를 찾을 수 없습니다", self.provider))
        })
    }
    ...
    pub fn get_provider(&self, name: &str) -> Result<&ProviderConfig> {
        self.providers
            .get(name)
            .ok_or_else(|| DoumError::Config(format!("프로바이더 '{}'를 찾을 수 없습니다", name)))
    }
