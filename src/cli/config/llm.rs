...
use crate::cli::config::{provider, context};
...
fn edit_timeout() -> Result<()> {
    ...
    let new_timeout = prompt_number_input::<u64>(
        "Timeout (seconds)",
        Some(config.llm.timeout)
    )?;
...
fn edit_retry() -> Result<()> {
    ...
    let new_retries = prompt_number_input::<u32>(
        "Maximum retry attempts",
        Some(config.llm.max_retries)
    )?;
...
fn toggle_thinking() -> Result<()> {
    ...
    println!("✅ Use thinking: {}", if config.llm.use_thinking { "enabled" } else { "disabled" });
...
fn toggle_web_search() -> Result<()> {
    ...
    println!("✅ Web search: {}", if config.llm.use_web_search { "enabled" } else { "disabled" });
