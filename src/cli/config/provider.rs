...
use crate::system::config::{ProviderConfig, load_config, save_config};
    ...
    loop {
        let menu = Menu::builder("Providers Configuration")
            ...
            .add_item(
                "openai",
                "OpenAI",
                "Configure OpenAI provider (API key, model, etc.)",
            )
            .add_item(
                "anthropic",
                "Anthropic",
                "Configure Anthropic provider (API key, model, etc.)",
            )
            .build();
...
fn run_openai_config() -> Result<()> {
    loop {
        ...
        let (model, api_key_display) =
            if let ProviderConfig::Openai(openai_config) = provider_config {
                ...
                return Err(DoumError::Config(
                    "OpenAI configuration not found".to_string(),
                ));
            };
        ...
        let menu = Menu::builder("OpenAI Configuration")
            ...
            .add_item("project", "Project (optional)", "Set project ID (optional)")
            .build();
    ...
    loop {
        ...
        let (model, api_key_display) =
            if let ProviderConfig::Anthropic(anthropic_config) = provider_config {
                ...
                return Err(DoumError::Config(
                    "Anthropic configuration not found".to_string(),
                ));
            };
...
fn edit_openai_model() -> Result<()> {
    ...
    let models = vec![
        ...
        (
            "gpt-5-mini",
            "GPT-5 Mini",
            "Faster and cheaper version of GPT-5",
        ),
        (
            "gpt-5-nano",
            "GPT-5 Nano",
            "Fastest and most affordable for simple tasks",
        ),
    ...
    let menu = Menu::builder("Select OpenAI Model").with_back().build();
    ...
    let model = match selected {
        ...
        Some(item) if item.id == "custom" => prompt_text_input("Model name", Some(&current_model))?,
...
fn edit_anthropic_model() -> Result<()> {
    ...
    let models = vec![
        ...
        (
            "claude-opus-4-1",
            "Claude Opus 4.1",
            "Exceptional for specialized reasoning",
        ),
        ("custom", "Custom", "Enter model name manually"),
    ];

    let menu = Menu::builder("Select Anthropic Model").with_back().build();
