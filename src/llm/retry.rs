    ...
    P: Fn(&str) -> Result<T>,
{
    ...
    for attempt in 1..=max_retries {
        ...
        let response = match llm_call().await {
            ...
            Err(e) => {
                ...
                if attempt < max_retries {
                    tracing::warn!("LLM call failed (attempt {}/{}): Retrying...", attempt, max_retries);
                    continue;
                } else {
        ...
        match parser(&response) {
            ...
            Err(e) => {
                last_error = Some(e);
                if attempt < max_retries {
                    tracing::warn!("Parse failed (attempt {}/{}): Retrying...", attempt, max_retries);
                    continue;
                } else {
                    tracing::error!("All retry attempts exhausted.");
                    break;
    ...
    Err(last_error.unwrap_or_else(|| {
        DoumError::LLM("Unknown error after retries".to_string())
    }))
}
